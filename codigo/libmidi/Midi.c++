...
Midi Midi::ReadFromStream(std::istream &stream)
{
	Midi m;

	// header_id is always "MThd" by definition
	const static std::string MidiFileHeader = "MThd";
	const static std::string RiffFileHeader = "RIFF";
	...
	stream.read(header_id, static_cast<std::streamsize>(MidiFileHeader.length()));
...
const std::vector<MidiTrack> &Midi::Tracks() const
{
	return m_tracks;
}
...
microseconds_t Midi::GetSongPositionInMicroseconds() const
{
	return m_microsecond_song_position;
}

microseconds_t Midi::GetDeadAirStartOffsetMicroseconds() const
{
	return m_microsecond_dead_start_air;
}

const MidiEventMicrosecondList & Midi::GetBarLines() const
{
	return m_bar_line_usecs;
}

Midi::Midi(): m_initialized(false), m_microsecond_dead_start_air(0)
{
	Reset(0, 0);
}
