...
MidiEventSimple::MidiEventSimple(unsigned char s, unsigned char b1, unsigned char b2) : status(s), byte1(b1), byte2(b2)
{
}
...
unsigned long MidiEvent::GetDeltaPulses() const
{
	return m_delta_pulses;
}

void MidiEvent::SetDeltaPulses(unsigned long delta_pulses)
{
	m_delta_pulses = delta_pulses;
}

unsigned char MidiEvent::StatusCode() const
{
	return m_status;
}
...
std::string MidiEvent::NoteName(unsigned int note_number)
{
	...
	const static std::string NoteBases[NotesPerOctave] =
	{
	...
	const std::string note_base = NoteBases[note_number % NotesPerOctave];
