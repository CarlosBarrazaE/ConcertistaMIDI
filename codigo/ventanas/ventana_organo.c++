	...
	for (MidiEventListWithTrackId::const_iterator i = eventos.begin(); i != eventos.end(); i++)
	{
		...
		else if((i->second.Type() == MidiEventType_NoteOn || i->second.Type() == MidiEventType_NoteOff) && i->second.NoteVelocity() == 0)
		{
				...
				i->second.NoteNumber() <= m_teclado_util.tecla_final())
			{
				if(m_pistas->at(i->first).modo() != Fondo)
				{
					unsigned long int nota_tocada = this->encontrar_nota_tocada(i->first, i->second.NoteNumber());
					if(nota_tocada < ULONG_MAX)
					{
						//Verifica si se paso de largo en una nota tocada correctamente
						if(!this->hay_nota_nueva(i->second.NoteNumber()))
						{
							std::map<unsigned int, Nota_Activa*>::iterator nota = m_notas_activas.find(i->second.NoteNumber());
							if(nota != m_notas_activas.end() && nota->second->correcta)
							{
								nota->second->color = Pista::Colores_pista[0];
								nota->second->correcta = false;
							}
						}
						this->desbloquear_nota(i->first, nota_tocada);
					}
					else
	...
	while(m_controlador_midi->hay_eventos())
	{
		...
		if(evento.tipo_evento() == EventoMidi_NotaEncendida && evento.velocidad() > 0)
		{
			...
			for(unsigned long int pista = 0; pista < m_pistas->size(); pista++)
			{
				...
				microseconds_t menor_tiempo = LLONG_MAX;
...
void VentanaOrgano::calcular_teclas_activas(unsigned int diferencia_tiempo)
{
	...
	for(unsigned int pista=0; pista<m_notas.size(); pista++)
	{
		...
		{
			for(unsigned long int n=m_primera_nota[pista]; n<m_notas[pista].size(); n++)
			{
...
unsigned long int VentanaOrgano::encontrar_nota_tocada(unsigned long int pista, unsigned int id_nota)
{
	...
	return ULONG_MAX;//No encontrado
}
...
void VentanaOrgano::desbloquear_notas(bool desbloquear_todas)
{
	...
	{
		unsigned int notas_revisadas = 0;
		for(unsigned long int pista = 0; pista < m_pistas->size() && (m_notas_bloqueadas-notas_revisadas) > 0; pista++)
		{
			//Se salta las pistas que no toca el jugador
			if(m_pistas->at(pista).modo() == Fondo)
				continue;
			...
			if(!desbloquear_todas)
				inicio = m_primera_nota[pista];

			for(unsigned long int n = inicio; n < pista_actual.size() && (m_notas_bloqueadas-notas_revisadas) > 0; n++)
			{
				if(pista_actual[n].tocada)
				{
					if(desbloquear_todas)
					{
					...
					else
					{
						...
						//Hay notas que no se desbloquearan porque estan siendo tocadas
						//pero no quiero recorrer el resto del ciclo
						notas_revisadas++;
					}
				}
			}
